...
const Stats = ({ projects = [], skills = [], testimonials = [] }) => {
  return (
    <section className="py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <CounterStat icon={FaProjectDiagram} label="Projects" value={projects.length} delay={0} />
            <CounterStat icon={FaCode} label="Skills" value={skills.length} delay={0.1} />
            <CounterStat icon={FaEnvelope} label="Happy Clients" value={50} delay={0.2} />
            <CounterStat icon={FaQuoteLeft} label="Testimonials" value={testimonials.length} delay={0.3} />
          </div>
        </FadeIn>
      </div>
    </section>
